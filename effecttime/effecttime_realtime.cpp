...
static DefaultGUIModel::variable_t vars[] = {
    ...
    {
        "First Pause (sec)",
        "How long the first pause should last (default = 0.01)",
        DefaultGUIModel::PARAMETER | DefaultGUIModel::DOUBLE,
    },
    {
        "Second Pause (sec)",
        "How long the second pause should last (default = 0.05)",
        DefaultGUIModel::PARAMETER | DefaultGUIModel::DOUBLE,
    },
    {
        "Increments",
        "Number of steps including the starting currents and the finishing currents, logarithmically spaced (default = 6)",
...
effecttime::effecttime(void)
    : DefaultGUIModel("effecttime",::vars,::num_vars), dt(RT::System::getInstance()->getPeriod()*1e-6),
      delay(2.0), initial_pulse(50.0), hyperpolarizing_pulse(-100.0), pulse_length(10.0), first_pause(0.01), second_pause(0.05), increments(6) {
    ...
    //This is for recording
      ...
      prefix = "effecttime";
      info = "no comment";
      path  = "/home/eric/Dropbox/Documents/School/rtxi/Modules/effecttime/data/";
      cellnum = 1;
      pdone = 0;
...
void effecttime::execute(void) {
    V = input(0);

    //Do all time keeping in seconds.
    if (step <= increments + 1) {
        if ((age > delay) && (age <= delay + step * pulse_length + totalpause - thispause)) {
            Iout = initial_pulse;
        }
        else if (age > (delay + step * pulse_length + totalpause - thispause) && age <= delay + step * pulse_length + totalpause && step != increments + 1) {
            ...
            step++;
            if (step % 2 != 0) {
                thispause=first_pause*pow(10,((step - 1) / 2));
            }
            else {
                thispause=second_pause*pow(10,step / 2 - 1);
            }
...
void effecttime::update(DefaultGUIModel::update_flags_t flag) 
{
    switch(flag) {
        case INIT:
            ...
            setParameter("First Pause (sec)",first_pause);
            setParameter("Second Pause (sec)",second_pause);
        ...
        case MODIFY:
            ...
            first_pause           = getParameter("First Pause (sec)").toDouble();
            second_pause          = getParameter("Second Pause (sec)").toDouble();
    ...
    // Setting initial totalpause length
    thispause=first_pause*pow(first_pause,(step + 1) / 2 - 1);
    totalpause += thispause;
}
