function [rate_all,numberspikes,imp,mean_v]=...
    sliding_rate(module,recdate,cellnum,trials,sample_rate,delay,pulse_length,...
    first_pause,second_pause,increments,windowsize,slidesize)
...
for h=1:increments+1;
    ...
    if h>1
        mean_v(h)=mean(trialdata(pulse_finish(h-1):pulse_start(h),1))*1e3;
    else
        mean_v=mean(trialdata(1:pulse_start(1)-1,1))*1e3;
    end
...
figure;plot(slidesize:slidesize:(pulse_length-windowsize),rate_all','LineWidth',2); hold on;
% title({['Initial Rate for Cell ' recdate '_' cellnum '_' num2str(trials)];...
%     ['Held at ' num2str(imp) ' mV; Delay: ' num2str(delay) ' sec; Pulse Duration: '...
%     num2str(pulse_duration) ' sec; Pause Duration: ' num2str(pause_duration) ' sec']},'interpreter','none')
