...
EL=-75;
slp=10;%# %2-5
Vr=-55;%%-65 default; -95 for slp = 1; -90 slp = 2; -90 slp = 5; -72 slp = 10
...
gBmax=50*10;%# % slow, potassium current
ENa=50; % mV
EK=-90;
% taua=0.016*1000;
% tauah=0.040*1000;
...
taubh=(7.36508751804256+142.286674746436*exp(-(-6.17390760853182-initv).^2/11.5357877928097^2))*1000;
...
I=700*ones(size(t)); % pause current; 150 for hyperpolarized, 600 for depolarized
for k=1:steps
    currentstep(k)=0+120*k;
...
%     ahinf=1/(1+exp((v(i)+85)/3));
    binf=1/(1+exp((v(i)+25.4)/-10.48));
    bhinf=1/(1+exp((v(i)+59.1)/4.66));
    taubh=(7.37+142.29.*exp(-(-6.17-v(i)).^2/11.54^2))*1000;
...
figure(1);hold on;plot(t,v,'Color',color);
...
hold on;
figure(3);hold on;plot(t(1:end-1),Ib,'Color',color);
% f-i curve
for k=1:steps
...
figure(2);hold on;plot(currentstep,firate,'Color',color)
title('f-I Curve')
xlabel('Current [pA]')
ylabel('Firing Rate [Hz]')

fifit=polyfit(currentstep,firate,1);
fifitline=polyval(fifit,currentstep);

% hold on;plot(currentstep,fifitline,'r')
