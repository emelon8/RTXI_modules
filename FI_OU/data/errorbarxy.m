function [oldlengthofxerrorbars,oldlengthofyerrorbars,varargout]=errorbarxy(lengthofxerrorbars,lengthofyerrorbars,varargin)
...
h=plot(x,y, color{1}); % main plot
if numel(lengthofxerrorbars)==1 && numel(lengthofyerrorbars)==1
    oldlengthofyerrorbars=ylim;
    oldlengthofxerrorbars=xlim;
    xerrorbar=0.01*(oldlengthofxerrorbars(2)-oldlengthofxerrorbars(1));
    yerrorbar=0.04*(oldlengthofyerrorbars(2)-oldlengthofyerrorbars(1));
else
    oldlengthofyerrorbars=ylim;
    oldlengthofxerrorbars=xlim;
    xerrorbar=0.01*(lengthofxerrorbars(2)-lengthofxerrorbars(1));
    yerrorbar=0.04*(lengthofyerrorbars(2)-lengthofyerrorbars(1));
end
allh=nan(length(x), 6); % all errorbar handles
for k=1:length(x)
    if ~isempty(lx) & ~isempty(ly) % both errors are specified
        l1=line([lx(k) ux(k)],[y(k) y(k)]);
        hold on;
        l2=line([lx(k) lx(k)],[y(k)-yerrorbar y(k)+yerrorbar]);
        l3=line([ux(k) ux(k)],[y(k)-yerrorbar y(k)+yerrorbar]);
        l4=line([x(k) x(k)],[ly(k) uy(k)]);
        l5=line([x(k)-xerrorbar x(k)+xerrorbar],[ly(k) ly(k)]);
        l6=line([x(k)-xerrorbar x(k)+xerrorbar],[uy(k) uy(k)]);
        allh(k, :)=[l1, l2, l3, l4, l5, l6];
    elseif isempty(lx) & ~isempty(ly) % x errors are not specified
        l4=line([x(k) x(k)],[ly(k) uy(k)]);
        hold on;
