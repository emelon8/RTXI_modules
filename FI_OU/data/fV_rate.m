function [rate_all,numberspikes,imp,pulse_duration,pause_duration,delay,increments,currents,...
...
%         pulse_finish{k}(h)=pulse_start{k}(h)+pulse_duration(k)*sample_rate-1;
        ...
        avg_voltage{k}(h)=mean(trialdata(pulse_start{k}(h)+2500:pulse_finish{k}(h)));
        skew_noise{k}(h)=skewness(trialdata(pulse_start{k}(h)+2500:pulse_finish{k}(h),1));
