function [ISI,rate_all,peakrate,nofailrate,mean_holdingvoltage,mean_r_m,std_r_m,mean_time_constant,std_time_constant,...
    mean_c_m,std_c_m,numberspikes,imp,pulse_duration,pause_duration,delay,increments,currents,pf_all,std_noise]=...
...
spiketimes=cell(1,howmany); thresholdspikes=cell(1,howmany); inflections=cell(1,howmany); inflectiontimes=cell(1,howmany);
thresholds=cell(1,howmany); incrementsegment=cell(1,howmany);
...
%     thresholdspikes{k}(diff(trialdata(:,1))>0.003)=1;
    ...
    for h=1:increments(k);
        ...
        % Find the ISIs
        if sum(spiketimes{k}(pulse_start{k}(h):pulse_finish{k}(h)))>1
...
%     end
    ...
    if shouldplotFI~=0
        if ~isequal(currents{k},0)
            ...
            title({['Rate for Cell ' recdate '_' cellnum '_' num2str(trials(k))];...
