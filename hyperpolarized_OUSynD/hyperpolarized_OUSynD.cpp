...
#include "hyperpolarized_OUSynD.h"
#include <iostream>
#include <sstream>
#include <stdlib.h>
#include <cmath>
...
extern "C" Plugin::Object *createRTXIPlugin(void) {
    return new hyperpolarized_OUSynD();
}
...
static DefaultGUIModel::variable_t vars[] = {
    ...
    {
        "Pause Current",
        "Current to hold the cell at during pauses (pA)",
        DefaultGUIModel::PARAMETER,
    },
    {
        "Repetitions",
        "Number of times to repeat the hyperpolarization followed by the OU process",
        DefaultGUIModel::PARAMETER,
    },
};
...
hyperpolarized_OUSynD::hyperpolarized_OUSynD(void)
    : DefaultGUIModel("hyperpolarized_OUSynD",::vars,::num_vars) { 
        ...
        pctDone = 0;
        downsample = 1;
        path = "/home/eric/Dropbox/Documents/School/rtxi/Modules/hyperpolarized_OUSynD/data/";
...
void hyperpolarized_OUSynD::execute(void) {
    ...
    if (step <= reps) {
        if (age >= (pauset + outime) * step - outime && age < (pauset + outime) * step) {
            ...
            sineI = gain*sin(2*M_PI*freq*tcnt);
...
void hyperpolarized_OUSynD::update(DefaultGUIModel::update_flags_t flag) 
{
    ...
    if (flag == INIT) {
        ...
        setParameter("Pause Current", pausei);
        setParameter("Repetitions", reps);
        ...
        pausei = getParameter("Pause Current").toDouble();
        reps = getParameter("Repetitions").toUInt();
...
void hyperpolarized_OUSynD::updateA() {
    // compute Asyn
    //Asyn = sqrt((Dsyn * tausyn / 2) * (1 - exp(-2 * dt / tausyn)));
	Dsyn=(STDsyn * STDsyn) / tausyn * 2;
