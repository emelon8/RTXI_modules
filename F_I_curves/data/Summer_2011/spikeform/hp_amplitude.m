function hp_amplitude(recdate,varargin)
if nargin>1
    cellnum=varargin{1};
else
...
hp_amp=cell(increments,trials_control+trials_propofol);
hp_amp_avg=zeros(increments,trials_control+trials_propofol);
hp_amp_ste=zeros(increments,trials_control+trials_propofol);
        ...
        if ~isempty(spiketimes{j,k})
            ...
            for p=1:length(spikeawesome{j,k})
                if (delay+pulse_duration)*sample_rate+((j-1)*(pulse_duration+pause_duration)-accom_time)*...
                    ...
                    eval(['hp_amp{j,k}(p)=' dataname{k} '((delay+pulse_duration)*sample_rate+((j-1)*'...
                        '(pulse_duration+pause_duration)-accom_time)*sample_rate+spikeawesome{j,k}(p),2)-min('...
                        dataname{k} '((delay+pulse_duration)*sample_rate+((j-1)*'...
...
%                         dataname{k} '((delay+pulse_duration)*sample_rate+((j-1)*'...
%                         '(pulse_duration+pause_duration)-accom_time)*sample_rate+spikeawesome{j,k}(p):'...
%                         '(delay+pulse_duration)*sample_rate+(j-1)*(pulse_duration+pause_duration)*sample_rate,2));'])
                end
            end
            if numel(hp_amp{j,k})>1
                hp_amp_avg(j,k)=mean(hp_amp{j,k}(find(hp_amp{j,k}~=0)));
                hp_amp_ste(j,k)=std(hp_amp{j,k}(find(hp_amp{j,k}~=0)))/...
                    sqrt(numel(hp_amp{j,k}(find(hp_amp{j,k}~=0))));
            else
                hp_amp_avg(j,k)=hp_amp{j,k};
                hp_amp_ste(j,k)=0;
            end
        end
    end
...
bargraph=[hp_amp_avg(1,1) hp_amp_ste(1,1); hp_amp_avg(1,2) hp_amp_ste(1,2);...
    hp_amp_avg(2,1) hp_amp_ste(2,1); hp_amp_avg(2,2) hp_amp_ste(2,2)];

save(['hp_amplitude_' recdate '_' cellnum],'rate','bargraph')

figure;bar(hp_amp_avg*1000)%,0.5)
set(gca,'XTickLabel',{[num2str(rate(1,1)) ' Hz and ' num2str(rate(2,1)) ' Hz'],...
    [num2str(rate(1,2)) ' Hz and ' num2str(rate(2,2)) ' Hz']},'FontSize',14)
title(['Hyperpolarization Amplitude for Control and ' conc_propofol ' \muM Propofol'],'fontsize',14)
legend('Control','Propofol')
ylabel('Hyperpolarization Amplitude [mV]','fontsize',12)
hold on
errorbar([0.855 1.145 1.855 2.145],bargraph(:,1)*1000,bargraph(:,2)*1000,'.','LineWidth',2.5)