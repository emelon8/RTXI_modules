...
for k=1:numel(dates)
    eval(['load hp_amplitude_' dates{k} '_' cellnum{k} '.mat;'])
...
[ttest_AHP(1),ttest_AHP(2)]=ttest(matrix_avg_control,matrix_avg_propofol);

figure;bar(means*1000,0.5,'b')
set(gca,'XTickLabel',{'Control','Propofol'},'FontSize',14)
title(['Effects of ' num2str(conc_propofol) ' \muM Propofol on AHP Amplitude at '...
    num2str(avg_rate_comparison) ' \pm 0.25 Hz'],'fontsize',16)
ylabel('AHP Amplitude [mV]','fontsize',12)
hold on
errorbar(means*1000,stes*1000,'.','LineWidth',2.5)