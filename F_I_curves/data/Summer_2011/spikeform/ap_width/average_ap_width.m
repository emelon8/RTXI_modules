...
for k=1:numel(dates)
    eval(['load ap_width_' dates{k} '_' cellnum{k} '.mat;'])
...
figure;bar(means/10,0.5,'b')
set(gca,'XTickLabel',{'Control','Propofol'},'FontSize',14)
title(['Effects of ' num2str(conc_propofol) ' \muM Propofol on AP Width at '...
    num2str(avg_rate_comparison) ' \pm 0.25 Hz'],'fontsize',16)
ylabel('AP Width [ms]','fontsize',12)
hold on
errorbar(means/10,stes/10,'.','LineWidth',2.5)