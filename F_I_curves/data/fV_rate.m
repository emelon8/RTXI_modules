function [rate_all,numberspikes,imp,pulse_duration,pause_duration,delay,increments,currents,pf_all,threshold,avg_voltage,avg_voltage_trunc]=...
...
pf_all=cell(1,howmany); imp=NaN(1,howmany);
...
%         pulse_finish{k}(h)=pulse_start{k}(h)+pulse_duration(k)*sample_rate-1;
        ...
        avg_voltage{k}(h)=mean(trialdata(pulse_start{k}(h)+2500:pulse_finish{k}(h)))*1000;
...
%         avg_voltage_trunc{k}=avg_voltage{k}(points(3):points(4));
%     elseif howmany==2 && mod(k,2)
%         all_trunc{k}=rate_all{k}(points(1):points(2));
%         avg_voltage_trunc{k}=avg_voltage{k}(points(1):points(2));
%     else
%         all_trunc{k}=rate_all{k}(find(rate_all{k}>0,1):end);
%         avg_voltage_trunc{k}=avg_voltage{k}(find(rate_all{k}>0,1):end);
%     end
    
    pf_all{k}=regstats(all_trunc{k},avg_voltage_trunc{k},'linear',{'beta' 'yhat' 'rsquare'});
    
%     pf_all(k,:)=polyfit(avg_voltage_trunc{k},all_trunc{k},1);
%     
%     linfit_all{k}=polyval(pf_all(k,:),avg_voltage_trunc{k});
%     
%     % Calculate the R^2 value for linear fit
%     resid_all_lin{k}=all_trunc{k}-linfit_all{k};
%     SSresid_all_lin(k)=sum(resid_all_lin{k}.^2);
%     SStotal_all_lin(k)=sum((all_trunc{k}-mean(all_trunc{k})).^2);%or (length(all_trunc{k})-1)*var(all_trunc{k});
%     rsq_all_lin(k)=1-SSresid_all_lin(k)/SStotal_all_lin(k);
