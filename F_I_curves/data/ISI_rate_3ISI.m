function [rate_all,ISI,rate_init,rate_accom,spikeform,imp,gain_ratio,...
    ...
    ISI_rate_3ISI(module,recdate,cellnum,trials,sample_rate,shouldplotFI,shouldplotISI,shouldplotspikeform)
...
%     thresholdspikes{k}(trialdata(:,1)>=-0.02)=1;
    ...
    for h=1:increments(k);
        ...
        if numel(ISI{k}{h})>3
            ...
            rate_accom{k}(h)=1./mean(ISI{k}{h}(end-2:end));
        elseif numel(ISI{k}{h})>0 && numel(ISI{k}{h})<=3
            rate_init{k}(h)=1./mean(ISI{k}{h}(1:end));
            rate_accom{k}(h)=1./mean(ISI{k}{h}(1:end));
        else
            rate_init{k}(h)=0;
            rate_accom{k}(h)=0;
        end
