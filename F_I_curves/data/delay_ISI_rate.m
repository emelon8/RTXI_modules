function [rate_all,ISI,rate_init,rate_accom,spikeform,imp,gain_ratio,...
    pulse_duration,pause_duration,delay,increments,currents,pf_delay,pf_init,...
    pf_accom,sigfit_delay,sigfit_init,sigfit_accom,sigslope_delay,sigslope_init,sigslope_accom]=...
...
% cell number (e.g. 'A'); TRIALS is a vector of the two trial numbers, the
% first being the hyperpolarized trial and the second being the depolarized
% trial; SAMPLE_RATE is the sample rate in Hz; SHOULDPLOTFI, SHOULDPLOTISI,
...
pulse_finish=cell(1,howmany); pulse_start=cell(1,howmany); accom_start=cell(1,howmany);
increment_spikes=cell(1,howmany); spikes=cell(1,howmany); spikeform=cell(1,howmany);
ISI=cell(1,howmany); ISI_delay=cell(1,howmany); ISI_accom=cell(1,howmany);
rate_all=cell(1,howmany); rate_delay=cell(1,howmany); rate_init=cell(1,howmany);
rate_accom=cell(1,howmany); delay_trunc=cell(1,howmany); init_trunc=cell(1,howmany);
currents_delay=cell(1,howmany); currents_init=cell(1,howmany); accom_trunc=cell(1,howmany);
currents_accom=cell(1,howmany); pf_delay=NaN(howmany,2); pf_init=NaN(howmany,2);
pf_accom=NaN(howmany,2); linfit_delay=cell(1,howmany); linfit_init=cell(1,howmany);
linfit_accom=cell(1,howmany); resid_delay_lin=cell(1,howmany); resid_init_lin=cell(1,howmany);
resid_accom_lin=cell(1,howmany); SSresid_delay_lin=NaN(1,howmany); SSresid_init_lin=NaN(1,howmany);
SSresid_accom_lin=NaN(1,howmany); SStotal_delay_lin=NaN(1,howmany); SStotal_init_lin=NaN(1,howmany);
SStotal_accom_lin=NaN(1,howmany); rsq_delay_lin=NaN(1,howmany); rsq_init_lin=NaN(1,howmany);
rsq_accom_lin=NaN(1,howmany); nlf_delay=NaN(howmany,3); nlf_init=NaN(howmany,3);
nlf_accom=NaN(howmany,3); sigfit_delay=cell(1,howmany); sigfit_init=cell(1,howmany);
sigfit_accom=cell(1,howmany); sigslope_delay=NaN(1,howmany); sigslope_init=NaN(1,howmany);
sigslope_accom=NaN(1,howmany); resid_delay_sig=cell(1,howmany); resid_init_sig=cell(1,howmany);
resid_accom_sig=cell(1,howmany); SSresid_delay_sig=NaN(1,howmany); SSresid_init_sig=NaN(1,howmany);
SSresid_accom_sig=NaN(1,howmany); SStotal_delay_sig=NaN(1,howmany); SStotal_init_sig=NaN(1,howmany);
SStotal_accom_sig=NaN(1,howmany); rsq_delay_sig=NaN(1,howmany); rsq_init_sig=NaN(1,howmany);
rsq_accom_sig=NaN(1,howmany); true_init_slope=NaN(1,howmany); true_accom_slope=NaN(1,howmany);
gain_ratio=NaN(1,howmany); gain_ratio_delay=NaN(1,howmany); gain_measure=cell(1,howmany);
imp=NaN(1,howmany);
...
%     thresholdspikes{k}(trialdata(:,1)>=-0.02)=1;
    ...
    for h=1:increments(k);
        ...
        ISI_delay{k}{h}=diff(find(diff(spiketimes{k}(pulse_start{k}(h)+...
            find(spiketimes{1}(pulse_start{k}(h):pulse_finish{k}(h))==1,1)-2:... % -2 because one extra is needed to catch the first spike
            pulse_finish{k}(h)))>0)+1)/sample_rate; % Finds the ISI, in seconds, of the depolarized case as if it had a delay
        ...
        if numel(ISI{k}{h})>0
            ...
            % Plot the rate as it changes from ISI to ISI during each pulse
            if shouldplotISI~=0
                figure(k*3-1);hold on;ISIplot=plot(1./ISI{k}{h});
                set(ISIplot,'Color',[0 (h-1)/increments(k) 1-(h-1)/increments(k)])
                title({'Change in Firing Rate Over Each Pulse in the Trial';...
                    [module '_' recdate '_' cellnum num2str(trials(k))]},'interpreter','none')
        ...
        if numel(ISI_delay{k}{h})>3
            rate_delay{k}(h)=1./mean(ISI_delay{k}{h}(1:3));
        elseif numel(ISI_delay{k}{h})>0 && numel(ISI_delay{k}{h})<4
            rate_delay{k}(h)=1./mean(ISI_delay{k}{h}(1:end));
        else
            rate_delay{k}(h)=0;
        end
        if numel(ISI{k}{h})>3
...
%         end
        ...
        if shouldplotspikeform~=0
            figure(k*3-2);hold on;spikeformplot=plot(0.1:0.1:(beforespike+afterspike+1)/10,spikeform{k}{h}*1000);
            set(spikeformplot,'Color',[0 (h-1)/increments(k) 1-(h-1)/increments(k)])
            title({'Steady-State Spike Waveform for Each Pulse in the Trial';...
                [module '_' recdate '_' cellnum num2str(trials(k))]},'interpreter','none')
            xlabel('Time [ms]')
    ...
    delay_trunc{k}=rate_delay{k}(find(rate_delay{k}>0,1):end);
    currents_delay{k}=currents{k}(find(rate_delay{k}>0,1):end);
...
%     currents_accom{k}=currents{k}(rate_accom{k}>0);
    ...
    pf_delay(k,:)=polyfit(currents_delay{k},delay_trunc{k},1);
    pf_init(k,:)=polyfit(currents_init{k},init_trunc{k},1);
    pf_accom(k,:)=polyfit(currents_accom{k},accom_trunc{k},1);
    
    linfit_delay{k}=polyval(pf_delay(k,:),currents_delay{k});
    ...
    resid_delay_lin{k}=delay_trunc{k}-linfit_delay{k};
    resid_init_lin{k}=init_trunc{k}-linfit_init{k};
    resid_accom_lin{k}=accom_trunc{k}-linfit_accom{k};
    
    SSresid_delay_lin(k)=sum(resid_delay_lin{k}.^2);
    SSresid_init_lin(k)=sum(resid_init_lin{k}.^2);
    SSresid_accom_lin(k)=sum(resid_accom_lin{k}.^2);
    
    SStotal_delay_lin(k)=(length(delay_trunc{k})-1)*var(delay_trunc{k});
    SStotal_init_lin(k)=(length(init_trunc{k})-1)*var(init_trunc{k});
    SStotal_accom_lin(k)=(length(accom_trunc{k})-1)*var(accom_trunc{k});
    
    rsq_delay_lin(k)=1-SSresid_delay_lin(k)/SStotal_delay_lin(k);
    rsq_init_lin(k)=1-SSresid_init_lin(k)/SStotal_init_lin(k);
    rsq_accom_lin(k)=1-SSresid_accom_lin(k)/SStotal_accom_lin(k);
    
    % Fit a sigmoid to the f-I curve and find the maximum slope of the
    % sigmoid
%     nlf_init(k,:)=nlinfit(currents{k},rate_init{k},'sigFun',[320,50,10]);
%     nlf_accom(k,:)=nlinfit(currents{k},rate_accom{k},'sigFun',[150,100,25]);
    if ~isequal(size(currents_delay{k}),[0 0])
        nlf_delay(k,:)=nlinfit(currents_delay{k},delay_trunc{k},'sigFun',[320,50,10]);
    end
    if ~isequal(size(currents_init{k}),[0 0])
        nlf_init(k,:)=nlinfit(currents_init{k},init_trunc{k},'sigFun',[320,50,10]);
    end
    if ~isequal(size(currents_accom{k}),[0 0])
        nlf_accom(k,:)=nlinfit(currents_accom{k},accom_trunc{k},'sigFun',[150,100,25]);
    end
    
%     sigfit_init{k}=sigFun(nlf_init(k,:),currents{k});
%     sigfit_accom{k}=sigFun(nlf_accom(k,:),currents{k});
    sigfit_delay{k}=sigFun(nlf_delay(k,:),currents_delay{k});
    sigfit_init{k}=sigFun(nlf_init(k,:),currents_init{k});
    sigfit_accom{k}=sigFun(nlf_accom(k,:),currents_accom{k});
    
    if ~isequal(rate_delay{k},zeros(size(rate_delay{k}))) && ~isequal(size(sigfit_delay{k}),[1 1])
        sigslope_delay(k)=max(diff(sigfit_delay{k}));
    else
        sigslope_delay(k)=0;
    end
    if ~isequal(rate_init{k},zeros(size(rate_init{k}))) && ~isequal(size(sigfit_init{k}),[1 1])
        sigslope_init(k)=max(diff(sigfit_init{k}));
    else
        sigslope_init(k)=0;
    end
    if ~isequal(rate_accom{k},zeros(size(rate_accom{k}))) && ~isequal(size(sigfit_accom{k}),[1 1])
        sigslope_accom(k)=max(diff(sigfit_accom{k}));
    else
        sigslope_accom(k)=0;
    end
    
    % Calculate the R^2 value for sigmoidal fit
%     resid_init_sig{k}=rate_init{k}-sigfit_init{k};
%     resid_accom_sig{k}=rate_accom{k}-sigfit_accom{k};
    resid_delay_sig{k}=delay_trunc{k}-sigfit_delay{k};
    resid_init_sig{k}=init_trunc{k}-sigfit_init{k};
    resid_accom_sig{k}=accom_trunc{k}-sigfit_accom{k};
    
    SSresid_delay_sig(k)=sum(resid_delay_sig{k}.^2);
    SSresid_init_sig(k)=sum(resid_init_sig{k}.^2);
    SSresid_accom_sig(k)=sum(resid_accom_sig{k}.^2);
    
%     SStotal_init_sig(k)=(length(rate_init{k})-1)*var(rate_init{k});
%     SStotal_accom_sig(k)=(length(rate_accom{k})-1)*var(rate_accom{k});
    SStotal_delay_sig(k)=(length(delay_trunc{k})-1)*var(delay_trunc{k});
    SStotal_init_sig(k)=(length(init_trunc{k})-1)*var(init_trunc{k});
    SStotal_accom_sig(k)=(length(accom_trunc{k})-1)*var(accom_trunc{k});
    
    rsq_delay_sig(k)=1-SSresid_delay_sig(k)/SStotal_delay_sig(k);
    rsq_init_sig(k)=1-SSresid_init_sig(k)/SStotal_init_sig(k);
    rsq_accom_sig(k)=1-SSresid_accom_sig(k)/SStotal_accom_sig(k);
...
%     else
        ...
        gain_ratio_delay(k)=pf_accom(k,1)/pf_delay(k,1);
%     end
    ...
    if shouldplotFI~=0
        if ~isequal(currents{k},0)
            ...
            legend('Original Values',['Linear Fit, Gain=' num2str(pf_init(k,1)) ', R^2=' num2str(rsq_init_lin(k))],...
                ['Sigmoidal Fit, Gain=' num2str(sigslope_init(k)) ', R^2=' num2str(rsq_init_sig(k))])
            xlabel('Current [pA]')
            ylabel('Firing Rate [Hz]')
            ...
            title({['Steady-State Rate for Cell ' cellnum ', Trial ' num2str(trials(k))];...
                ...
                num2str(pulse_duration(k)) ' sec; Pause Duration: ' num2str(pause_duration(k)) ' sec'];...
                ['Steady-State/Inital Gain Ratio (from ' gain_measure{k} '): ' num2str(gain_ratio(k))]})
            legend('Original Values',['Linear Fit, Gain=' num2str(pf_accom(k,1)) ', R^2=' num2str(rsq_accom_lin(k))],...
                ['Sigmoidal Fit, Gain=' num2str(sigslope_accom(k)) ', R^2=' num2str(rsq_accom_sig(k))])
            xlabel('Current [pA]')
            ylabel('Firing Rate [Hz]')
            axis([currents{k}(1) currents{k}(end) 0 150])
            
            figure;subplot(2,1,1);plot(currents{k},rate_delay{k},'*k')
            hold on;
            plot(currents_delay{k},linfit_delay{k},'r'); hold on;
            plot(currents_delay{k},sigfit_delay{k})
            title({['Delayed Initial Rate for Cell ' cellnum ', Trial ' num2str(trials(k))];...
                ['Held at ' num2str(imp(k)) ' mV; Delay: ' num2str(delay(k)) ' sec; Pulse Duration: '...
                num2str(pulse_duration(k)) ' sec; Pause Duration: ' num2str(pause_duration(k)) ' sec'];...
                ['Steady-State/Delay Gain Ratio (from ' gain_measure{k} '): ' num2str(gain_ratio_delay(k))]})
            legend('Original Values',['Linear Fit, Gain=' num2str(pf_delay(k,1)) ', R^2=' num2str(rsq_delay_lin(k))],...
                ['Sigmoidal Fit, Gain=' num2str(sigslope_delay(k)) ', R^2=' num2str(rsq_delay_sig(k))])
            xlabel('Current [pA]')
            ylabel('Firing Rate [Hz]')
            axis([currents{k}(1) currents{k}(end) 0 150])
        end
