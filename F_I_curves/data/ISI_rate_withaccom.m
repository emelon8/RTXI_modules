function [rate_all,ISI,rate_init,rate_accom,ISI_accom,spikeform,imp,gain_ratio,...
    pulse_duration,pause_duration,delay,increments,currents,pf_init,pf_accom,...
    sigfit_init,sigfit_accom,sigslope_init,sigslope_accom]=...
    ISI_rate_withaccom(module,recdate,cellnum,trials,sample_rate,shouldplotFI,shouldplotISI,shouldplotspikeform)
...
start_current=NaN(1,howmany); finish_current=NaN(1,howmany); accom_time=NaN(1,howmany);
increments=NaN(1,howmany); currents=cell(1,howmany);

% Preallocate vectors associated with rates
spiketimes=cell(1,howmany); thresholdspikes=cell(1,howmany); pulse_finish=cell(1,howmany);
pulse_start=cell(1,howmany); accom_start=cell(1,howmany); increment_spikes=cell(1,howmany);
...
gain_ratio=NaN(1,howmany); imp=NaN(1,howmany);
...
for k=1:howmany
    ...
    accom_time(k)=round((pulse_duration(k)/3)*sample_rate)/sample_rate;
...
%     thresholdspikes{k}(trialdata(:,1)>=-0.02)=1;
    ...
    for h=1:increments(k);
        ...
        if shouldplotspikeform~=0
            figure(k*4-3);hold on;spikeformplot=plot(0.1:0.1:(beforespike+afterspike+1)/10,spikeform{k}{h}*1000);
        ...
        if numel(ISI{k}{h})>0
            ...
            if shouldplotISI~=0
                figure(k*4-2);hold on;ISIplot=plot(1./ISI{k}{h});
...
%     ALTERNATIVE METHOD
    ...
    if shouldplotFI~=0
        if ~isequal(currents{k},0)
            figure(k*4-1);plot(currents{k},rate_init{k},'*k'); hold on;
            ...
            xlabel('Current [pA]')
            ylabel('Firing Rate [Hz]')
            axis([currents{k}(1) currents{k}(end) 0 350])
            figure(k*4);plot(currents{k},rate_accom{k},'.k'); hold on;
            ...
            xlabel('Current [pA]')
            ylabel('Firing Rate [Hz]')
            axis([currents{k}(1) currents{k}(end) 0 300])
        end
    end
end